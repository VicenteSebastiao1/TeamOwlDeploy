...
def base(request):
    ...
    return render(request, 'base.html', {'home': True, "c": user_profile_data})
...
def myclub(request):
    ...
    post_data = Post.objects.filter(post_author=user_info).select_related('post_author',
                                                                          'post_author__userprofile').order_by(
        "-date_posted")
...
def myposts(request):
    ...
    seven_days_post_data = Post.objects.filter(date_posted__gte=seven_day_before, post_author=user_info).select_related(
        'post_author', 'post_author__userprofile').order_by("-date_posted")
...
def homefeed(request):
    ...
    seven_days_post_data = Post.objects.filter(date_posted__gte=seven_day_before).select_related('post_author',
                                                                                                 'post_author__userprofile').order_by(
        "-date_posted")
        ...
        try:
            ...
            if user_data.exists():
                ...
                if user is not None:
                    ...
                    request.session['user_id'] = user_data[0].username
                    request.session.set_expiry(0)
                    print('2', dict(request.session))
...
def getClubByName(request):
    ...
    club_set = Club.objects.filter(club_name__contains=name).all()
    club_list = []
    for i in club_set:
        print(i.id, "i.id")
        data = {
            "club_picture": i.club_picture,
            "club_name": i.club_name,
            "club_genre": i.club_genre,
            "club_description": i.club_description,
            "club_location": i.club_location,
            "eighteen_plus": i.eighteen_plus,
            "average_club_rating": i.average_club_rating,
            "club_owner": i.club_name,
        }
        club_list.append(data)
    user_info = request.user
    user_profile_data = UserProfile.objects.get(user=user_info.id)

    return render(request, "feedclub.html", locals())
=======
